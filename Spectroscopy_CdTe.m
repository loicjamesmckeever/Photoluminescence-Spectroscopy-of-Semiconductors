 data=xlsread('CdTe_Peak.xlsx','A16:B2062');
 
 wavelength=data(:,1);
 intensity=data(:,2);
 energy=1239.513.*wavelength.^-1;
 figure(1)
 ...
 peak_eV=1239.513/peak_nm;
 
 FWHM_nm=fwhm(wavelength,intensity);
 FWHM_eV=abs(fwhm(energy,intensity));
 
 error_nm=FWHM_nm/2.3548;
 error_eV=FWHM_eV/2.3548;
 
 hbar=6.5821195144*10^-16;
 mo=0.510998946131*10^6/299792458^2;
 m=0.09*mo;
 eo=1.49;
 d=sqrt((hbar^2*pi^2)/(2*m*(peak_eV-1.49)));
 d_nm=d*10^9;
 error_d=((((hbar^2*pi^2)/(2*m*(peak_eV-1.49)))/(2*d))*(error_eV/(peak_eV-1.49)^2));
 error_d_nm=error_d*10^9;
